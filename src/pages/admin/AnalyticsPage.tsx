  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Revenue', value: `OMR ${totalRevenue.toLocaleString()}` },
          { label: 'Avg Order Value', value: `OMR ${avgOrderValue.toFixed(1)}` },
        ...
        <div className="space-y-3">
          {topProducts.length === 0 ? (
            ...
            <div key={p.name} className="flex items-center justify-between py-2 border-b border-border last:border-0">
              ...
              <span className="font-body text-sm font-bold text-primary">OMR {p.revenue}</span>
            </div>
          ))}
        </div>
      </div>
