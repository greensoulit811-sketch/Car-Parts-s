            ...
            {filtered.map(lead => {
              ...
              return (
                <tr key={lead.id} className="border-b border-border last:border-0 hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="p-4">
                    <p className="font-body text-sm font-bold text-foreground">OMR {Number(lead.cart_total).toFixed(3)}</p>
                  </td>
                  <td className="p-4">
          ...
          <div className="bg-card border border-border w-full max-w-lg my-8 p-6 md:p-8 rounded-lg shadow-lg">
            ...
            <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    {((viewingLead.cart_items as any[]) || []).map((item: any, i: number) => (
                      <div key={i} className="flex justify-between font-body text-sm py-1.5 border-b border-border last:border-0">
                        ...
                        <span className="font-semibold text-foreground">OMR {((item.price || 0) * (item.quantity || 1)).toFixed(3)}</span>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <div className="flex justify-between items-center border-t border-border pt-4">
                ...
                <span className="font-heading text-lg font-bold text-primary">OMR {Number(viewingLead.cart_total).toFixed(3)}</span>
              </div>
