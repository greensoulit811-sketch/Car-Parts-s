...
const BannersManager = () => {
  ...
  const [form, setForm] = useState({ title: '', subtitle: '', image_url: '', link_url: '/parts', is_active: true, position: 'hero' as string });
  const [uploading, setUploading] = useState(false);
  const imageRef = useRef<HTMLInputElement>(null);

  const openAdd = () => { setEditing(null); setForm({ title: '', subtitle: '', image_url: '', link_url: '/parts', is_active: true, position: 'hero' }); setShowForm(true); };
  const openEdit = (b: DbBanner) => { setEditing(b); setForm({ title: b.title, subtitle: b.subtitle || '', image_url: b.image_url, link_url: b.link_url || '/parts', is_active: b.is_active ?? true, position: b.position }); setShowForm(true); };
