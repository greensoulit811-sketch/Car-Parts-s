      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          <Link to="/" className="flex items-center gap-2">
            {s?.logo_url ? (
              <img src={s.logo_url} alt={s?.site_name || 'Legacy-29'} width="150" height="48" className="h-10 lg:h-12 w-auto object-contain" />
            ) : (
          ...
          <div className="hidden md:flex items-center gap-8 font-body text-[13px] tracking-widest uppercase font-semibold text-foreground/80">
            ...
            <Link to="/parts" className="relative py-2 group hover:text-neon transition-colors duration-300">
              {t('nav.parts')}
              <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-neon transition-all duration-300 group-hover:w-full"></span>
            </Link>
            {topCategories.map(cat => (
              <Link key={cat.id} to={`/parts?category=${cat.slug}`} className="relative py-2 group hover:text-neon transition-colors duration-300">
                {cat.name}
                <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-neon transition-all duration-300 group-hover:w-full"></span>
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 md:gap-3 text-foreground">
            <Link to="/parts" className="relative flex items-center justify-center w-10 h-10 rounded-full bg-[#F4F4F4] hover:bg-gray-200 text-[#111] transition-all duration-300 group" aria-label="Search">
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex flex-col px-6 py-6 gap-2 font-body text-sm tracking-widest uppercase font-medium text-foreground">
              ...
              <Link to="/parts" onClick={() => setMobileOpen(false)} className="hover:text-neon transition-colors py-3 border-b border-border/40">{t('nav.parts')}</Link>
              {topCategories.map(cat => (
                <Link key={cat.id} to={`/parts?category=${cat.slug}`} onClick={() => setMobileOpen(false)} className="hover:text-neon transition-colors py-3 border-b border-border/40">
                  {cat.name}
                </Link>
              ))}
